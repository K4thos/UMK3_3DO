...
// CD ASYNC CHECKING
static int  sync_intr = 0;
static char sync_result[8] = {0,0,0,0,0,0,0,0};
...
static int psxcd_critical_sync(void)
{
    unsigned long block;
...
void psxcd_disable_callbacks(void)
{
#if _CD_VERSION_ == 1
    cb_enable_flag = 0;
#endif
...
void psxcd_enable_callbacks(void)
{
#if _CD_VERSION_ == 1
    cb_enable_flag = 1;
#endif
...
int psxcd_seeking_for_play(void)
{
#if _CD_VERSION_ == 1
    if(seeking_for_play)
    {
...
int psxcd_waiting_for_pause(void)
{
#if _CD_VERSION_ == 1
...
int psxcd_read(void *destptr,int readbytes,PsxCd_File *fileptr)
{
    int retbytes;
...
void psxcd_async_read_cancel(void)
{
#if _CD_VERSION_ == 1
    if(async_on)
    {
...
int psxcd_async_read(void *destptr,int readbytes,PsxCd_File *fileptr)
{
#if _CD_VERSION_ == 1
...
int psxcd_seek(PsxCd_File *fileptr,int seekpos,int seekmode)
{
#if _CD_VERSION_ == 1
...
unsigned long psxcd_tell(PsxCd_File *fileptr)
{
#if _CD_VERSION_ == 1
...
void psxcd_close(PsxCd_File *fileptr)
{
#if _CD_VERSION_ == 1
...
void psxcd_set_audio_mode(void)
{
#if _CD_VERSION_ == 1
    if(psxcd_mode!=0)
    {
...
void psxcd_play_at(int track, int vol, int sectoroffset)
{
#if _CD_VERSION_ == 1
...
void psxcd_play(int track, int vol)
{
    psxcd_play_at(track,vol,0);
}

void psxcd_seek_for_play_at(int track, int sectoroffset)
{
#if _CD_VERSION_ == 1
...
void psxcd_seek_for_play(int track)
{
...
void psxcd_set_stereo(int stereo_true)
{
#if _CD_VERSION_ == 1
    if(stereo_true)
    {
